...
def minhaCallback(arquivo):
    if variavelEtapa1(arquivo, listaPcerto):
        variavelEtapa2(arquivo, listaErrado)

def verificaLinha(Regex_parada,String_ArquivoNome,int_linha, callback_verificacao=None):
#vai verificar se há erros até o regex de parada(begin)
    ...
    while Regex_parada.search(arquivo) == None:
        callback_verificacao(arquivo)#Uso de callback
        arquivo = Meu_arquivo.readline()
        int_linha += 1
...
for linha in range(linhaTxt):
    ...
    if minhaLinha != None:
        erros = verificaLinha(fim, "hello.txt",linha,minhaCallback)
        if len(erros[0]) > 0:
